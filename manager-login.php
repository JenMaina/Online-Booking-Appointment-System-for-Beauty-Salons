...
else
{
    ...
    $query = mysqli_query($conn,"SELECT * FROM manager WHERE password='$password' AND username='$username'");
    $rows = mysqli_fetch_assoc($query);
    $num=mysqli_num_rows($query);
    if ($num == 1) {
        $_SESSION['username']=$rows['username'];
        $_SESSION['mgrname']=$rows['name'];
        $_SESSION['mgrid']=$rows['mid'];
        echo "Logging you in..";
       echo '<script type="text/javascript">setTimeout(function(){window.top.location="manager-page.php"} , 1000);</script>';
    } 
    else 
    {
...
<a href="javascript:history.back()">
 <button class="btn btn-icon waves-effect waves-light btn-danger pull-right" data-toggle="tooltip" title="Close"> <i class="fa fa-remove"></i> </button>
            ...
            <div class="panel-heading">
                <br> 
                <h3 class="text-center"> Sign In as <strong class="text-custom">Manager</strong> </h3>
            </div> 


            <div class="panel-body">
            <form class="form-horizontal m-t-20" action="manager-login.php" method="post">
                ...
                <div class="col-sm-12 text-center">
                    <p>Don't have an account? <a href="page-register-manager.php" class="text-primary m-l-5" data-toggle="tooltip" title="Sign Up"><b>Sign Up</b></a></p>
