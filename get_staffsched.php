...
<body>
                                        <?php
                                              ...
                                              $upload_dir = 'uploads/';
                                                $sql = "SELECT * FROM staff WHERE stfid = '". $_POST['staff'] ."'";
                                                $result = mysqli_query($conn, $sql);
                                                if (mysqli_num_rows($result) > 0) {
                                                  $row1 = mysqli_fetch_assoc($result);
                                                  $_SESSION['stfidsched'] = $row1['stfid'];
                                                }else {
...
<?php
include "dbconfig.php";
    $query ="SELECT * FROM staff_availability WHERE stfid = '". $_POST['staff'] ."' ORDER BY fro ASC";
    $result = $conn->query($query);
    while($row = mysqli_fetch_array($result)) {
        ?>
        <div class="radio radio-success" >
        <input type="radio" value="<?php echo $row['fro'] ?>" name="tym" required="">
        <label for="radio4">
